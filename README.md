...
- 签到端
  ...
  - 对于签到次数过多的账号，将会自动停用该账号
    - 签到失败天数达到3天
  - 提供两个脚本：
    - `auto_sign.py`：可选择Mysql或Sqlite3，但实际上签到是同步进行，正常情况下，约50s-1min完成一个人的签到任务
    - `auto_sign_sync.py`：只支持Mysql，可自行配置多用户异步签到，速度翻倍。
- 注册信息端
  - 提供Web页面向用户提供注册功能，将用户提交的信息保存至数据库供签到端使用
  - 为用户提供取消签到功能，用户可自行选择是否启用自动签到（是否停用）
  - 可通过API向网页推送通知信息

> 虽然全部使用异步方式编写，但签到脚本并没有发挥异步的优势，需要完全重写，因此如果把每个人签到视为一次签到任务的话，实际运行结果是任务排队执行，效率极低，这也限制了在有限时间内实际支持的签到人数。
> 
> [关于使用异步方式编写的说明](#开发说明)
...
> 1.6.0版本之后，`auto_sign.py`强制使用Mysql以支持多用户并发签到功能，配置文件中的`choose`值只会影响到`auto_sign_sync.py`和`web_app.py`，因此如果想使用sqlite3，请使用`auto_sign_sync.py`脚本并将配置文件中的`choose`配置为`sqlite`
...
项目代码基本使用异步方式编写，为的是实现多个用户同时签到，但实际上签到脚本并没有发挥异步的优势，需要对其重写。需要把签到任务细分成对不同API的请求，例如完整地帮助一个用户完成签到，需要经历约八九个请求，将这些请求视为异步任务等待，让脚本在这些脚本间自动切换才能最大化利用请求的空档期来提示效率，但显然当前的签到脚本并没有做到这点。
